    ...
    <div className="fixed inset-0 bg-destructive/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-md shadow-2xl bg-card text-card-foreground">
        <CardHeader className="text-center">
          <CardTitle className="text-3xl text-destructive-foreground bg-destructive p-3 rounded-t-md -m-6 mb-3">Game Over!</CardTitle>
          <CardDescription className="text-lg pt-3">Your economic journey has ended.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 text-center">
          ...
          {gameOverData.failedQuestion && (
            <div className="mt-4 pt-3 border-t border-border text-left p-3 bg-secondary rounded-md">
              <p className="font-semibold text-secondary-foreground">Regarding the last question:</p>
              <p className="text-sm text-muted-foreground">"{gameOverData.failedQuestion.questionText}"</p>
              <p className="text-sm mt-1">The correct answer was: <strong className="text-primary">{gameOverData.failedQuestion.correctAnswerText}</strong></p>
              {gameOverData.failedQuestion.explanationText && (
                 <p className="text-xs mt-1 text-muted-foreground"><em>Explanation: {gameOverData.failedQuestion.explanationText}</em></p>
              )}
            </div>
          )}
        </CardContent>
